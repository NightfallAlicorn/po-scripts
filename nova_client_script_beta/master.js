/*
Name: master.js
Sha1: 20ebf9923128819789a110fb079881fb9dad6db0
...
var CONFIG = {
    ...
    noticeIgnoreEnabled: false,
    welcomeMessage: "<img src='pokemon:num=359-1&gen=6&shiny=false&back=false'>"
};

var CONSTANTS = {
    configFile: BASEDIR + "config.json",
    sha1Auto: sys.sha1("auto.js"),
    sha1Master: sys.sha1("master.js"),
    pluginDir: BASEDIR + "plugins/",
    pluginDataDir: BASEDIR + "plugin_data/",
    pluginServerUrl: "https://raw.githubusercontent.com/NightfallAlicorn/po-scripts/master/nova_client_script_beta/plugin_server.json",
    scriptAutoUrl: "https://raw.githubusercontent.com/NightfallAlicorn/po-scripts/master/nova_client_script_beta/auto.js",
    scriptStandardUrl: "https://raw.githubusercontent.com/NightfallAlicorn/po-scripts/master/nova_client_script_beta/master.js"
};
...
var NOVA_C = {
	privateCommands: function (command, commandData, channelName, channelId) {
        ...
        if (command === "help" || command === "commands") {
            ...
            if (commandData === "settings") {
                ...
                helpArray = [
                    ...
                    "nonotice [on/off]: Blocks Pokémon Online's automatic /notice ads.",
                    ...
                    "updatescript [standard/auto]: Update this client script. Auto will automatically download each time you start PO so you always got latest version."
                ];
            }
        ...
        if (command === "nochallenge") {
			...
			sendBotHtml("No challenge currently: <b>" + (CONFIG.ignoreChallenge ? "on" : "off") + "</b>");
			return;
		}
        if (command === "nonotice") {
            if (commandData === "on") {
                CONFIG.noticeIgnoreEnabled = true;
                sendBotHtml("No notice <b>on</b>.");
                saveConfig();
                return;
            }
            if (commandData === "off") {
                CONFIG.noticeIgnoreEnabled = false;
                sendBotHtml("No notice <b>off</b>");
                saveConfig();
                return;
            }
            sendBotHtml("No notice currently: <b>" + (CONFIG.noticeIgnoreEnabled ? "on" : "off") + "</b>");
            return;
        }
        ...
        if (command === "updatescript") {
            ...
            try {
                if (commandData === "standard") {
                    ...
                    sys.webCall(CONSTANTS.scriptStandardUrl, function (response) {
                        sys.writeToFile(sys.scriptsFolder + "scripts.js", response);
                        sys.changeScript(response, true);
                        sendBotHtml("Script updated.");
                        return;
                    });
                }
                if (commandData === "auto") {
...
var PLUGINS = {
            ...
            if (this.cache.hasOwnProperty(x)) {
                if (this.cache[x][event] !== undefined) {
                    try {
                        ...
                        delete this.cache[x];
                        sendMasterText("Plugin error, " +  x + " been disabled, " + error.message);
                    }
    ...
    disable: function (plugin) {
        ...
        CONFIG.pluginEnabledArray.remove(plugin);
        if (this.cache[plugin] !== undefined) {
            delete this.cache[plugin];
            sendBotHtml("<b>" + plugin.htmlEscape() + "</b> plugin disabled.");
        } else {
            sendBotHtml("<b>" + plugin.htmlEscape() + "</b> plugin isn't enabled.");
        }
        saveConfig();
        return;
    },
    ...
    enable: function (plugin) {
        plugin = plugin.toLowerCase();
        if (this.cache[plugin] !== undefined) {
            sendBotHtml("<b>" + plugin.htmlEscape() + "</b> plugin already enabled.");
            return;
        }
        if (sys.filesForDirectory(CONSTANTS.pluginDir).contains(plugin)) {
            this.cache[plugin] = require(CONSTANTS.pluginDir + plugin);
            CONFIG.pluginEnabledArray.add(plugin);
            sendBotHtml("<b>" + plugin.htmlEscape() + "</b> enabled.");
        } else {
            sendBotHtml("<b>" + plugin.htmlEscape() + "</b> isn't installed.");
        }
        saveConfig();
        return;
    },
    ...
    unInstall: function (plugin) {
        ...
        if (sys.fileExists(CONSTANTS.pluginDir + plugin)) {
            sys.deleteFile(CONSTANTS.pluginDir + plugin);
            sendBotHtml("<b>" + plugin.htmlEscape() + "</b> plugin uninstalled.");
        } else {
            sendBotHtml("<b>" + plugin.htmlEscape() + "</b> plugin isn't installed.");
        }
        saveConfig();
        return;
    },
...
if (client.ownId() > -1 && ISINIT === false) {
    ISINIT = true;
    loadConfig();
    PLUGINS.loadAll();
    welcomeMessage();
...
script = { // PO CREATES DUPLICATE IF NOT USING script AS NAME
    ...
    beforeChallengeReceived: function (challengeId, opponentId, tier, clauses) {
        ...
        if (CONFIG.ignoreChallenge) {
            sys.stopEvent();
            sendMasterHtml("Auto refused <b>" + client.name(opponentId).htmlEscape() + "</b>'s challenge.");
    ...
    beforeChannelMessage: function (fullMessage, channelId, isHtml) {
        ...
        if (isHtml) {
            ...
            if (fullMessage.indexOf("<timestamp/><b><span style") > -1) {
                var message = fullMessage.htmlStrip();
                if (message.indexOf(": ") > -1) {
                    var rainbowName = message.substring(0, message.indexOf(": "));
            ...
            // BLOCK /notice ADS
            if (CONFIG.noticeIgnoreEnabled && fullMessage.indexOf("<!-- Don't change above -->") > -1) {
                if (!TEMP.noNoticeSkip) {
                    sys.stopEvent();
                    return;
                }
                TEMP.noNoticeSkip = false;
            }
        ...
        /*
        var x,
            channelDisplay = "____________________",
            channelColor = (["Victory Road", "Indigo Plateau"].contains(channelName) ? "#ff3030" : "#aaaaaa");
        channelDisplay = channelName + channelDisplay.substring(channelName.length, channelDisplay.length);
        channelDisplay = channelDisplay.substring(0, 10);
        if (src !== -1 && channelName !== CONFIG.masterChannelName && client.hasChannel(client.channelId(CONFIG.masterChannelName))) {
            printHtml("[<b><span style='background-color: " + channelColor + "; font-family: Courier;'>" + channelDisplay + "</span></b>] <span style='color: " + userSentColor + "'><b>" + srcName.htmlEscape() + ":</b></span> " + srcMessage.htmlEscape(), client.channelId(CONFIG.masterChannelName));
        }
        */
    },
    ...
    beforeSendMessage: function (message, channelId) {
        ...
        if (["/", "!"].indexOf(message.charAt(0)) > -1) {
            serverCommand = message.substr(1).toLowerCase().trim();
            if (serverCommand === "notice") {
                TEMP.noNoticeSkip = true;
            }
        }
        ...
        if (CONFIG.removeCaps && command.length === 0 && message.toLowerCase() !== message) {
            sys.stopEvent();
